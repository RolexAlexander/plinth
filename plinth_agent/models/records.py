...
class OpenQuestion(BaseModel):
    ...
    @model_validator(mode="before")
    @classmethod
    def default_blocking_rationale(cls, v: Any) -> Any:
        if isinstance(v, dict):
            if v.get("blocking") is True:
                rat = v.get("blocking_rationale")
                if not rat or not str(rat).strip():
                    v["blocking_rationale"] = f"This question blocks requirements: {', '.join(v.get('affects') or []) or 'unspecified'}."
        return v
